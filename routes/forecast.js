...
// This function contains the "AI" logic. It's a simple set of rules.
...
router.get('/forecast', async (req, res) => {
    ...
    // --- FINAL FIX: Use precise Latitude and Longitude for Agra ---
    // This is much more reliable than using a city name.
    const lat = 27.1767; // Latitude for Agra
    const lon = 78.0081; // Longitude for Agra
    const url = `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${apiKey}&units=metric`;

    try {
        const response = await axios.get(url);
        const weatherMain = response.data.weather[0].main; // e.g., "Clear", "Rain"
